...
let toa teams id = team_out teams id [] 
...
let build_flow_network team_x other_teams = 
  ...
  let games_to_play = games other_teams in 
...
let calculate_max_flow initial_graph source_id sink_id =
  let string_graph_initial = g_to_string_int initial_graph in
  let string_graph_residual = ffalgo string_graph_initial source_id sink_id in
  let max_flow = 
    List.fold_left (fun acc arc ->
      let original_capacity = arc.lbl in
      match find_arc string_graph_residual arc.src arc.tgt with
      | Some residual_arc ->
        let residual_capacity = int_of_string residual_arc.lbl in
        acc + (original_capacity - residual_capacity)
      | None -> acc (* Should not happen if graph structure is preserved *)
    ) 0 (out_arcs initial_graph source_id)
  in
  (max_flow, string_graph_residual)

let check_elimination teams team_to_check_id = 
  let team_x = find_team teams team_to_check_id in 
  let other_teams = toa teams team_to_check_id in 
  if facile_elim team_x other_teams then true 
  else let (final_graph, source_id, sink_id, total_games_flow) = build_flow_network team_x other_teams in 
    let (max_flow, _) = calculate_max_flow final_graph source_id sink_id in 
    interpret_max_flow max_flow total_games_flow team_to_check_id 

let () = 
  if Array.length Sys.argv <> 3 then begin 
    printf "\nUsage: %s infile team_id_to_check\n\n" Sys.argv.(0); 
  ...
  let _ = check_elimination teams team_to_check_id in ()
