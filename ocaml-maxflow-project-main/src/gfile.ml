...
let from_file path =
  ...
  let export (filename: string) (graph: string graph) =
  let oc = open_out filename in
  (* Écrire l'en-tête du fichier DOT *)
  Printf.fprintf oc "digraph G {\n";
  
  (* Parcourir les arcs du graphe et les écrire au format DOT *)
  e_iter graph (fun arc ->
    (* Exclure les arcs avec des capacités négatives *)
    if not (String.contains arc.lbl '-') then
